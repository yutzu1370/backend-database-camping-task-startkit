...
select 
	"CREDIT_PURCHASE".user_id	as ID,
	sum("CREDIT_PURCHASE".purchased_credits) as 購買總堂數total 
...
select 
	"COURSE_BOOKING".user_id	as ID,
	count("COURSE_BOOKING".id)  as 已使用堂數total 
from "COURSE_BOOKING"  
...
select "BUY".user_id,
       ("BUY".total_buy-"USED".total_used)as 剩餘課堂數
from (
--計算王小明總購買堂數
select user_id,
	   sum(purchased_credits) as total_buy
from "USER" 
...
--計算王小明已使用堂數
select user_id,
	   sum(course_id) as total_used
from "USER" 
inner join "COURSE_BOOKING" on "USER".id ="COURSE_BOOKING".user_id
where "USER".email='wXlTq@hexschooltest.io' and "COURSE_BOOKING".cancelled_at is null
group by user_id 
) as "USED"
on "BUY".user_id="USED".user_id;
